...
def plot_fit(i_exp, SHUFFLE_SAMPLES, num_episodes, episode_predicted_networks, episode_max_possible_rewards, episode_star_max_possible_rewards, episode_rewards, episode_durations, is_eval=False):
    ...
    star_regret = np.cumsum(
        np.array(episode_star_max_possible_rewards) - np.array(episode_rewards))

    plt.plot(np.cumsum(regret), label="Cumulative regret")
    envtype = "random" if SHUFFLE_SAMPLES else "sequential"
    plt.plot(x_steps, x_steps, label="Linear cumulative regret")
    plt.plot(x_steps, star_regret, label="Episode star cumulative regret")
    plt.title(f"DQN ({envtype} samples)")
    plt.xlabel("Episode number")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--resume", type=int, required=False)
    ...
    # pprint(list(paramgrid)[:10])


    for i_param, param_set in tqdm(enumerate(paramgrid)):
        if args.resume is not None and i_param < args.resume:
            continue
        try: 
            ...
            episode_star_max_possible_rewards = []
            episode_durations = []
            ...
            for i_episode in range(num_episodes):
                ...
                for i in range(80):
                    ...
                    if done:
                        ...
                        episode_star_max_possible_reward = sum(
                            env.history_star_max_possible_rewards)
                        ...
                        episode_star_max_possible_rewards.append(
                            episode_star_max_possible_reward)
            ...
            plot_fit(i_param, param_set['step_sampling'], num_episodes, episode_predicted_networks,
                    episode_max_possible_rewards, episode_star_max_possible_rewards, episode_rewards, episode_durations)
            ...
            eval_episode_star_max_possible_rewards = []
            eval_episode_durations = []

            def eval(eval_episode_predicted_networks, eval_episode_best_true_networks, eval_episode_true_networks_r2, eval_episode_rewards, eval_episode_max_possible_rewards, eval_episode_star_max_possible_rewards, eval_episode_durations):
                policy_net.eval()
                # print(env.datasets.shape[0])
                for i_episode in range(env.r_test.shape[0]):
                    # Initialize the environment and state
                    ...
                    for i in range(80):
                        ...
                        if done:
                            ...
                            eval_episode_true_networks_r2.append(
                                env.current_rewards)
                            eval_episode_star_max_possible_reward = sum(
                                env.history_star_max_possible_rewards)
                            ...
                            eval_episode_star_max_possible_rewards.append(
                                eval_episode_star_max_possible_reward)
                            eval_episode_durations.append(i + 1)
                            break

            eval(eval_episode_predicted_networks, eval_episode_best_true_networks, eval_episode_true_networks_r2,
                eval_episode_rewards, eval_episode_max_possible_rewards, eval_episode_star_max_possible_rewards, eval_episode_durations)

            eval_episode_predicted_networks = np.nan_to_num(
                eval_episode_predicted_networks, copy=False)
            eval_episode_best_true_networks = np.nan_to_num(
                eval_episode_best_true_networks, copy=False)
            eval_episode_true_networks_r2 = np.nan_to_num(
                eval_episode_true_networks_r2, copy=False)
            eval_episode_rewards = np.nan_to_num(eval_episode_rewards, copy=False)
            eval_episode_max_possible_rewards = np.nan_to_num(
                eval_episode_max_possible_rewards, copy=False)
            eval_episode_star_max_possible_rewards = np.nan_to_num(
                eval_episode_star_max_possible_rewards, copy=False)
            eval_episode_durations = np.nan_to_num(
                eval_episode_durations, copy=False)

            plot_fit(i_param, param_set['step_sampling'], num_episodes, eval_episode_predicted_networks,
                    eval_episode_max_possible_rewards, eval_episode_star_max_possible_rewards, eval_episode_rewards, eval_episode_durations, is_eval=True)
        except KeyboardInterrupt:
            break
        except:
            continue

        with open(os.path.join(RESULTS_PATH, *["trace_partial.txt"]), "a") as f:
            f.write(f"{i_param}:"+str(trace_experiment[i_param])+"\n")
